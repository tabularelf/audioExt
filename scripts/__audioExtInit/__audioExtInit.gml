...
function __audioExtInit() {
	...
	if !(_initated) {
		...
		if (AUDIO_EXT_SCAN_ON_START) {
			// Ogg first
			audioExtOggScan(AUDIO_EXT_DIR_OGG_TO_SCAN_ON_START, AUDIO_EXT_PRELOAD_ON_START);
			
			// Now Wav
			audioExtWavScan(AUDIO_EXT_DIR_WAV_TO_SCAN_ON_START, AUDIO_EXT_PRELOAD_ON_START, AUDIO_EXT_COMPRESS_ON_START);
		}
	}
}
